...
1. refreshBeanFactory(): 抽象方法，子类【AbstractRefreshableApplicationContext】唯一实现的：
            ...
            <mark>（这里面需要注意的是：.register()只是把当前这一个Class对象registry.registerBeanDefinition()了，至于内部的@Bean、@ComponentScan扫描到的，都不是在此处注册的）</mark>
...
6. **beanFactory.preInstantiateSingletons()**;实例化后剩下的单实例bean(过程这里就不详说了)
